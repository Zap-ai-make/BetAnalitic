...
/** POST /api/beta/matches/[matchId]/analyze?home_team=...&away_team=... */
export async function POST(
  ...
  const body = await req.json() as { agent_type: string; question?: string }

  const res = await betaFetch(
    `/api/matches/${matchId}/analyze?${qs.toString()}`,
