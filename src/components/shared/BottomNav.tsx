...
const NAV_ICONS: { id: NavItem; icon: React.ReactNode }[] = [
  {
    id: "home",
    icon: (
...
export function BottomNav({ activeItem, onNavigate }: BottomNavProps) {
  ...
  const labels: Record<NavItem, string> = {
    ...
    analysis: t.nav.signals,
    paris: t.nav.bets,
    salles: t.nav.rooms,
  }

  const activeIndex = NAV_ICONS.findIndex((item) => item.id === activeItem)
      ...
      <div className="relative flex items-center justify-around h-16">
        ...
        <div
          ...
          style={{
            ...
            left: `${(activeIndex / NAV_ICONS.length) * 100}%`,
        ...
        {NAV_ICONS.map((item) => {
          ...
          return (
            <button
              ...
              style={{ transform: isTapped ? "scale(1.15)" : "scale(1)" }}
              aria-current={isActive ? "page" : undefined}
            >
              <span className={cn("transition-colors duration-200", isActive ? "text-accent-cyan" : "text-text-tertiary")}>
                {item.icon}
              </span>
              <span className={cn("text-xs font-medium transition-colors duration-200", isActive ? "text-accent-cyan" : "text-text-tertiary")}>
                {labels[item.id]}
              </span>
            </button>
          )
